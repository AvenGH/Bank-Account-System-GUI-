...
class AccountSystem:
    ...
    @staticmethod
    def fields_blank(name, email, password, conf_password, opening_balance):
        return name.get() == "" or email.get() == "" or password.get() == "" or conf_password.get() == "" or opening_balance.get() == ""
    ...
    @staticmethod
    def opening_balance_valid(amount):
        if not dc.is_positive(amount):
    ...
    def login(email, password):
        try:
            ...
            user = AccountAPI.fetch_account_by_email(email.get())
            return user
        except Exception as e:
    ...
    @classmethod
    def sign_up(cls, name, email, password, conf_password, opening_balance):
        ...
        if not cls.opening_balance_valid(opening_balance.get()):
            return
        ...
        try:
            ...
            accid = generate_random_id.generate_account_id()
            accno = generate_random_id.generate_account_no()
            pin = str(random.randrange(1000, 10000))

            afh.write_account_details(
                account_no=accno,
                name=name.get(),
                pin=pin,
                sort_code="40-39-13",
            ...
            AccountAPI.add_account(
                ...
                account_no=accno, 
                name=name.get(), 
                opening_balance=float(opening_balance.get()), 
                sort_code="40-39-13", 
                PIN=pin,
                attempts=3,
                blocked=False,
                email=email.get()
            ) # Adds the customer record to the database
            return True
